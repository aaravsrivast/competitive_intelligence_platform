...
class SocialMediaPostModel:
    ...
    async def ensure_indexes(cls) -> None:
        ...
        await col.create_index([("tenant_id", 1), ("posted_at", -1)])
    ...
    @classmethod
    async def get_by_id(cls, tenant_id: str, post_id: str) -> Optional[dict[str, Any]]:
        try:
    ...
    @classmethod
    async def delete(cls, tenant_id: str, post_id: str) -> bool:
        try:
            oid = ObjectId(post_id)
            tid = ObjectId(tenant_id)
        except Exception:
    ...
    @classmethod
    async def generate_highlights(cls, tenant_id: str, post_id: str) -> str:
        from services.openai_service import generate_key_highlights

        post = await cls.get_by_id(tenant_id, post_id)
        if not post:
        ...
        await cls._col().update_one(
            {"_id": ObjectId(post_id), "tenant_id": ObjectId(tenant_id)},
            {
                "$set": {
                    "highlights": text,
