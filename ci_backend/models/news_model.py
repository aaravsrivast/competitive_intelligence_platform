...
class NewsModel:
    ...
    @classmethod
    async def get_by_id(cls, tenant_id: str, article_id: str) -> Optional[dict[str, Any]]:
        try:
    ...
    @classmethod
    async def delete(cls, tenant_id: str, article_id: str) -> bool:
        try:
            oid = ObjectId(article_id)
            tid = ObjectId(tenant_id)
        except Exception:
    ...
    @classmethod
    async def generate_highlights(cls, tenant_id: str, article_id: str) -> str:
        from services.openai_service import generate_key_highlights

        article = await cls.get_by_id(tenant_id, article_id)
        if not article:
        ...
        await cls._col().update_one(
            {"_id": ObjectId(article_id), "tenant_id": ObjectId(tenant_id)},
            {
                "$set": {
                    "highlights": text,
